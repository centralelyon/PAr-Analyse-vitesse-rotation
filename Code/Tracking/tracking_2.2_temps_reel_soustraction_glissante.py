...
lower = (47, 41, 106) # blanche soustraction première frame
upper = (101, 143, 201)
...
#génération de la première liste d'images pour permettre la soustraction
...
while True:
    ...
    frames.appendleft(frame)
    processedFrame = frames[fenetre//2]
    fond=calcImageMoyenneArr(frames)
    
    processedFrameSub=cv2.subtract(processedFrame,fond)
    ...
    processedFrameSubResized = cv2.resize(processedFrameSub, dim, interpolation = cv2.INTER_AREA)
    cv2.imshow("sub",processedFrameSubResized)
    #floutage pour éliminer les effets des hautes fréquences
    #blurred=cv2.GaussianBlur(frame,(5,5),0)
    #cv2.imshow("blurred",blurred)
    
    #convert to hsv
    hsv=cv2.cvtColor(processedFrameSubResized,cv2.COLOR_BGR2HSV)
    ...
    if len(cnts):
        ...
        if radius < 100 and not (center is None): #j'ai enlevé la condition "radius >3". Changer ce critère
